...
public:
    ...
    void do_evaporation( ) {
        // Pas de dépendance entre cellules → parallélisation directe.
        // La boucle extérieure est sur i (lignes), répartie entre threads.
        // La boucle intérieure reste séquentielle (bonne localité cache par ligne).
    ...
    // ── Nouvelles méthodes pour la version MPI ────────────────────────────

    /**
     * @brief Évaporation sur une plage de lignes [i_start, i_end)
     * @details Permet à chaque processus MPI de traiter uniquement sa bande
     *          de la carte, répartissant ainsi la charge de l'évaporation.
     */
    void do_evaporation_range( std::size_t i_start, std::size_t i_end ) {
        #pragma omp parallel for schedule(static)
    ...
    /**
     * @brief Fusionne les buffers phéromone de tous les processus MPI (MAX)
     * @details Chaque processus a mis à jour le buffer pour ses fourmis.
     *          MPI_Allreduce avec MPI_MAX garantit que chaque case prend
     *          la valeur maximale vue par n'importe quel processus,
     *          conformément à l'énoncé du projet.
     *          Après cet appel, tous les processus ont le même buffer.
     */
#ifdef USE_MPI
    void allreduce_buffer_max() {
        // Le buffer est un vector<array<double,2>> → vu comme vector<double>
